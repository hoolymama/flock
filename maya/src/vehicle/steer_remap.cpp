...

{
	...
	st = addAttribute( aReference); msert;
	...
	tAttr.setWritable(true);
	tAttr.setCached(false);
	st = addAttribute( aGoal); msert;

	aRotMethod = eAttr.create( "calculateRotation", "clr", steerRemap::kMult, &st ); msert;
	...
	aMagMethod = eAttr.create( "calculateMagnitude", "clm", steerRemap::kMult, &st ); msert;
	eAttr.addField( "remap",		steerRemap::kRemap);
	eAttr.addField( "multiply",		steerRemap::kMult);
	eAttr.setKeyable(true);
	eAttr.setStorable(true);
	...
	st = addAttribute(aMagCurve ); mser;

	aRotCurve = rAttr.createCurveRamp("rotation", "rot", &st);
	st = addAttribute(aRotCurve ); mser;

	aOutput = tAttr.create("output", "o", MFnData::kVectorArray);
	tAttr.setStorable(false);
	tAttr.setReadable(true);
	st = addAttribute( aOutput ); msert;

	st = attributeAffects( aReference, 		aOutput ); msert;
	st = attributeAffects( aGoal, 			aOutput ); msert;

	st = attributeAffects( aRotMethod, 		aOutput ); msert;
	st = attributeAffects( aMagMethod, 		aOutput ); msert;

	st = attributeAffects( aRotCurve, 		aOutput); msert;
	st = attributeAffects( aMagCurve, 		aOutput); msert;
...
MStatus steerRemap::compute(const MPlug &plug, MDataBlock &data)
{
	...
	if (plug == aOutput) {
		...
		MRampAttribute rotRamp( thisMObject(), aRotCurve, &st ); msert;
		MRampAttribute magRamp( thisMObject(), aMagCurve, &st ); msert;
		//////////////////////////////////////////////////////////////
		// MFnAnimCurve rAniFn;
		// MFnAnimCurve mAniFn;
		...
		if ((rotMethod == steerRemap::kMult)
		    && (magMethod == steerRemap::kMult)) { // multiply both
			...
			for (unsigned i = 0; i < nLength; i++) {
				...
				rotRamp.getValueAtPosition( angleBetween, rotVal, &st ); mser;
				magRamp.getValueAtPosition( angleBetween, magVal, &st ); mser;
		...
		else if ((rotMethod == steerRemap::kRemap)
		         && (magMethod == steerRemap::kRemap)) {  // remap both
			//cerr  << "here 16_B"<<endl;
			for (unsigned i = 0; i < nLength; i++) {
		...
		else if ((rotMethod == steerRemap::kMult)
		         && (magMethod == steerRemap::kRemap)) {  // remap mag, multiply rot
			//cerr  << "here 16_C"<<endl;
			for (unsigned i = 0; i < nLength; i++) {
				...
				result[i] = newVec * magVal;
		         ...
		         && (magMethod == steerRemap::kMult))  { // remap  rot, multiply mag
			//cerr  << "here 16_D"<<endl;
			for (unsigned i = 0; i < nLength; i++) {
