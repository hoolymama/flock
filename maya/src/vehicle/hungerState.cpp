...
//
{
	...
	aDefaultFeed = nAttr.create("defaultFeed","dfd", MFnNumericData::kDouble, 1.0, &st ); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	addAttribute(aDefaultFeed);

	
	aDefaultSpend = nAttr.create("defaultSpend","dsp", MFnNumericData::kDouble, 0.1, &st ); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	addAttribute(aDefaultSpend);
	
	aFeed = tAttr.create("feed", "fed", MFnData::kDoubleArray, &st ); er;
	tAttr.setStorable(false);
	addAttribute(aFeed);
	
	aSpend = tAttr.create("spend", "spd", MFnData::kDoubleArray, &st ); er;
	tAttr.setReadable(true);
	tAttr.setStorable(false);
	addAttribute(aSpend);
	
	aFoodLevel = tAttr.create("foodLevel", "fl", MFnData::kDoubleArray, &st ); er;
	tAttr.setStorable(false);
	addAttribute(aFoodLevel);
	
	aUpdatedFoodLevel = tAttr.create("updatedFoodLevel", "ufl", MFnData::kDoubleArray, &st ); er;
	tAttr.setReadable(true);
	tAttr.setStorable(false);
	addAttribute(aUpdatedFoodLevel);
		
	aHungry = tAttr.create("hungry", "hg", MFnData::kDoubleArray, &st ); er;
	tAttr.setStorable(false);
	addAttribute(aHungry);
	
	aUpdatedHungry = tAttr.create("updatedHungry", "uhg", MFnData::kDoubleArray, &st ); er;
	tAttr.setReadable(true);
	tAttr.setStorable(false);
	addAttribute(aUpdatedHungry);	
	
	aHungryFullCycle = tAttr.create("hungryFullCycle", "hfc", MFnData::kDoubleArray, &st ); er;
	tAttr.setReadable(true);
	tAttr.setStorable(false);
	...
	aFullThreshold = nAttr.create("fullThreshold","fth", MFnNumericData::kDouble, 10.0, &st ); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	addAttribute(aFullThreshold);
	
	aHungryFullRamp = MRampAttribute::createCurveRamp("hungryFullRamp","hfr");
	st = addAttribute( aHungryFullRamp );er;	
	...
	st = attributeAffects(aCurrentTime, aUpdatedFoodLevel);er;
	st = attributeAffects(aCurrentTime, aUpdatedHungry);er;
	st = attributeAffects(aCurrentTime, aHungryFullCycle);er;
	
 // st = attributeAffects(aFeed, aUpdatedFoodLevel);er;
 // st = attributeAffects(aFeed, aUpdatedHungry);er;
 // st = attributeAffects(aSpend, aUpdatedFoodLevel);er;
 // st = attributeAffects(aSpend, aUpdatedHungry);er;
 // st = attributeAffects(aDefaultFeed, aUpdatedFoodLevel);er;
 // st = attributeAffects(aDefaultFeed, aUpdatedHungry);er;
 // st = attributeAffects(aDefaultSpend, aUpdatedFoodLevel);er;
 // st = attributeAffects(aDefaultSpend, aUpdatedHungry);er;
...
//	cerr << "foodLevel.length();  " << fl << endl;
	...
	//		cerr << " i " << i << "not hungry spend[i] " << spend[i] << endl;	
		...
		float rampVal = 0.0;
		if (doRamp) {
			rampAttr.getValueAtPosition( rampSample, rampVal, &st ); er;
		} else {
			rampVal = rampSample;
		}
	...
	MDataHandle hHungryFullCycle = data.outputValue(hungerState::aHungryFullCycle, &st );er;
	MFnDoubleArrayData fnHungryFullCycle;
	MObject dHungryFullCycle = fnHungryFullCycle.create( hungryFullCycle, &st ); er;
	hHungryFullCycle.set( dHungryFullCycle );


	MDataHandle hUpdatedFoodLevel = data.outputValue(hungerState::aUpdatedFoodLevel, &st );er;
	MFnDoubleArrayData fnUpdatedFoodLevel;
	MObject dUpdatedFoodLevel = fnUpdatedFoodLevel.create( updatedFoodLevel, &st ); er;
	hUpdatedFoodLevel.set( dUpdatedFoodLevel );
	
	
	MDataHandle hUpdatedHungry = data.outputValue(hungerState::aUpdatedHungry, &st );er;
	MFnDoubleArrayData fnUpdatedHungry;
	MObject dUpdatedHungry = fnUpdatedHungry.create( updatedHungry, &st ); er;
