...

{
  ...
  aRotMethod = eAttr.create( "calculateRotation", "clr", legacySteerRemap::kMult, &st );
  msert;
  ...
  aMagMethod = eAttr.create( "calculateMagnitude", "clm", legacySteerRemap::kMult, &st );
  msert;
  eAttr.addField( "remap",    legacySteerRemap::kRemap);
  eAttr.addField( "multiply",   legacySteerRemap::kMult);
  eAttr.setKeyable(true);
  ...
  st = addAttribute( aMagCurve ); msert;
  ...
  nAttr.setWritable(true);
  nAttr.setKeyable(true);
  st = addAttribute( aRotCurve ); msert;
...
MStatus legacySteerRemap::compute(const MPlug &plug, MDataBlock &data)
{
  ...
  if (plug == aOutput) {
    ...
    if ((rotMethod == legacySteerRemap::kMult)
        && (magMethod == legacySteerRemap::kMult)) { // multiply both
    ...
    else if ((rotMethod == legacySteerRemap::kRemap)
             && (magMethod == legacySteerRemap::kRemap)) {// remap both
      //cerr  << "here 16_B"<<endl;
      for (unsigned i = 0; i < nLength; i++) {
    ...
    else if ((rotMethod == legacySteerRemap::kMult)
             && (magMethod == legacySteerRemap::kRemap)) {// remap mag, multiply rot
      //cerr  << "here 16_C"<<endl;
      for (unsigned i = 0; i < nLength; i++) {
        ...
        result[i] = newVec * magVal;
             ...
             && (magMethod == legacySteerRemap::kMult))  {// remap  rot, multiply mag
      //cerr  << "here 16_D"<<endl;
      for (unsigned i = 0; i < nLength; i++) {
