...
////  nAttr.setStorable(true);
////  nAttr.setDefault(1.0);
////  st = addAttribute(aArchRangeMax);er;
//
//
//	aOutArchPP = tAttr.create("outArchPP", "oapp", MFnData::kDoubleArray , &st ); er;
//	tAttr.setStorable(false);
...
//	st = addAttribute(aOutArchPP);er;
	...
	st = addAttribute( aThrustFacingStrategy );er;
	
	
	aThrustFacingLookup = MRampAttribute::createCurveRamp("thrustFacingLookup","thlt");
	st = addAttribute( aThrustFacingLookup );er;	
  ...
  //  addAttribute (aSimulationType);
	...
	nAttr.setStorable(true);
	nAttr.setDefault(1.0);
	st = addAttribute(aEffortSpan);er;

	aOutEffortPP = tAttr.create("outEffortPP", "oepp", MFnData::kDoubleArray , &st ); er;
	tAttr.setStorable(false);
	tAttr.setReadable(true);
	st = addAttribute(aOutEffortPP);er;
	////////////////////////////////////////////////////////////	
...
//tAttr.setStorable(false);
//st = addAttribute(aFlapOffsetPP);er;
	...
	nAttr.setStorable(true);
	nAttr.setDefault(1.0);
	st = addAttribute(aFlapOffsetMult);er;	

	aFlapOffsetLookup = MRampAttribute::createCurveRamp("flapOffsetLookup","folt");
	st = addAttribute( aFlapOffsetLookup );er;	

	aOutFlapOffsetPP = tAttr.create("outFlapOffsetPP", "ofop", MFnData::kDoubleArray , &st ); er;
	tAttr.setStorable(false);
	tAttr.setReadable(true);
	st = addAttribute(aOutFlapOffsetPP);er;
	////////////////////////////////////////////////////////////	
	...
	nAttr.setStorable(true);
	nAttr.setDefault(1.0);
	st = addAttribute(aFlapAmplitudeMult);er;	

	aFlapAmplitudeLookup = MRampAttribute::createCurveRamp("flapAmplitudeLookup","falt");
	st = addAttribute( aFlapAmplitudeLookup );er;	

	aOutFlapAmplitudePP = tAttr.create("outFlapAmplitudePP", "ofap", MFnData::kDoubleArray , &st ); er;
	tAttr.setStorable(false);
	tAttr.setReadable(true);
	st = addAttribute(aOutFlapAmplitudePP);er;
	////////////////////////////////////////////////////////////	
	...
	st = attributeAffects(aCurrentTime  , aOutBendPP			);er;
	st = attributeAffects(aCurrentTime  , aOutArchPP 			);er;
	st = attributeAffects(aCurrentTime  , aOutEffortPP 			);er;
	st = attributeAffects(aCurrentTime  , aOutFlapOffsetPP 		);er;
	st = attributeAffects(aCurrentTime  , aOutFlapAmplitudePP 	);er;
...
//////////////////////////////////////////////////////////////////////////	
	...
	MVectorArray viewPP = fnViewPP.array( &st ); //er;
	...
	MVectorArray deltaViewPP = fnDeltaViewPP.array( &st ); //er;
	...
	MVectorArray upVectorPP = fnUpVectorPP.array( &st ); //er;
//////////////////////////////////////////////////////////////////////////	
...
//////////////////////////////////////////////////////////////////////////	
	...
	MDoubleArray bendPP = fnBendPP.array( &st ); //er;
	...
	MDoubleArray deltaBendPP = fnDeltaBendPP.array( &st ); //er;
...
//////////////////////////////////////////////////////////////////////////	
	...
	MDoubleArray archPP = fnArchPP.array( &st ); //er;
	...
	MDoubleArray deltaArchPP = fnDeltaArchPP.array( &st ); //er;
...
//////////////////////////////////////////////////////////////////////////	
	...
	MVectorArray thrustPP = fnThrustPP.array( &st );er;
	...
	MVectorArray velocityPP = fnVelocityPP.array( &st );er;	
//////////////////////////////////////////////////////////////////////////	
	...
	// MDoubleArray flapOffsetPP = fnFlapOffsetPP.array( &st );er;
	//////////////////////////////////////////////////////////////////////////
	...
	ThrustFacingStrategy thrustStrategy = ThrustFacingStrategy(data.inputValue(aThrustFacingStrategy).asShort());
	...
	st = calculateEffort(thrustPP, viewNorm, velocityPP, thrustMax, velocityMax, effortSpan, effortAttack,  effortDecay, lastEffortPP,outEffortPP, doThrust, doDrag, thrustStrategy); 

// now we have effort - calc flap offset
	MDoubleArray outFlapOffsetPP(len);
	MDoubleArray outFlapAmplitudePP(len);	
	if (doFlap){
		st = doRampLookup(thisNode, aFlapOffsetLookup, outEffortPP, outFlapOffsetPP,0.0f,1.0f,0.0f, float(flapOffsetMult)); 	er;
		st = doRampLookup(thisNode, aFlapAmplitudeLookup, outEffortPP, outFlapAmplitudePP,0.0f,1.0f,0.0f, float(flapAmplitudeMult)); 	er;
	}
	...
	if (doBend) {	
		st = calculateBend(viewNorm, deltaViewPP, upVectorPP, bendPP,   deltaBendPP,  bendImpulsePP, outBendPP, bendMagnitude, conserveBend, bendSpring); 
	}
	
	
	MDoubleArray  outArchPP(len);
	if (doArch) {	
		st = calculateArch(viewNorm, deltaViewPP, upVectorPP, archPP,   deltaArchPP,  archImpulsePP, outArchPP, archMagnitude, conserveArch, archSpring); 
		// 	st = calculateArch(viewPP, deltaViewPP, upVectorPP ,outArchPP , archMagnitude); er;
	}
	...
	MObject dOutEffortPP = fnOutEffortPP.create( outEffortPP, &st );er;
	...
	MObject dOutFlapOffsetPP = fnOutFlapOffsetPP.create( outFlapOffsetPP, &st );er;
	...
	MObject dOutFlapAmplitudePP = fnOutFlapAmplitudePP.create( outFlapAmplitudePP, &st );er;
	...
	MObject dOutBendPP = fnOutBendPP.create( outBendPP, &st );er;
	...
	MObject dOutArchPP = fnOutArchPP.create( outArchPP, &st );er;
	...
	) const 
{
	...
	st = doRampLookup(thisMObject(), articulate::aArchLookup, angles, lutResult, -1.5707963267948966,  1.5707963267948966,-archMagnitude,archMagnitude ); 	er;
...
//	}
	...
	st = doRampLookup(thisMObject(), articulate::aBendLookup, angles, lutResult, 0, 3.141592653,0.0,float(bendMagnitude)); 	er;
	...
	) const 
{
	...
	if (doThrust) {
		...
		if (thrustStrategy != articulate::kOff){
			...
			st = doRampLookup(thisMObject(), articulate::aThrustFacingLookup, thrustFacing, thrustValues, oldMin, 1.0f,0.0f,1.0f); er;
		}
	...
	if (doDrag) {	
		...
		st = doRampLookup(thisMObject(), articulate::aDragLookup, velocityLengths, dragValues, 0.0f, 1.0f,0.0f,1.0f); er;
 ...
 // 		cerr << "dragValues[i] " << dragValues[i] << endl;
	// }
	if (  (lastEffortPP.length() == len)  && ( (effortAttack != 1.0) ||  (effortDecay != 1.0)  ) ) {
		for (unsigned i=0;i<len;i++) {
...
{
	MStatus st;
	MRampAttribute rampAttr( node, attribute, &st ); ert;
	...
	{	
		...
		float res;
		rampAttr.getValueAtPosition( v, res, &st ); er;
