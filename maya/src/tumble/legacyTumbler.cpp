...
//
{
  ...
  st = addAttribute(aCollisionGoalRange); mser;

  aCollisionGoalWeightMap = MRampAttribute::createCurveRamp("collisionGoalWeightMap",
                            "cgwt");
  st = addAttribute( aCollisionGoalWeightMap ); mser;
  ...
  nAttr.setStorable(true);
  nAttr.setDefault(1.0);
  st = addAttribute(aImpulseGoalRange); mser;
  ...
  nAttr.setStorable(true);
  nAttr.setDefault(1.0);
  st = addAttribute(aVelocityGoalRange); mser;
...
)
{
  ...
  for (unsigned i = 0; i < len; i++) {
    ...
    if (doVelRamp) {
      ...
      velRampAttr.getValueAtPosition( in, rampVal, &st ); mser;
      gw *= rampVal;
      //  cerr << "gw " << gw << endl;
    }
    ...
    if (doImpulse) {
      if (doImpRamp) {
        ...
        impRampAttr.getValueAtPosition( in, rampVal, &st ); mser;
        gw *= rampVal;
      }
    }
    ...
    if (doCollisionImpulse) {
      if (doColRamp) {
        ...
        colRampAttr.getValueAtPosition( in, rampVal, &st ); mser;
        gw *= rampVal;
      }
    }
