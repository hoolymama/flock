...

{
		...
		(plug == aOut) )
	{
		...
		MDataHandle hOut =  data.outputValue( aOut, &st); er;
		MFnVectorArrayData fnOutput;
		MObject dOutput = fnOutput.create( outRotations, &st );er;
		hOut.set( dOutput);
		data.setClean( plug );
	}
	return MS::kSuccess;
...

{
	...
	aFront = tAttr.create("frontVector", "fnt", MFnData::kVectorArray , &st ); er;
    tAttr.setStorable(false);
	st = addAttribute( aFront ); er;
	
	aUp = tAttr.create("up", "upVector", MFnData::kVectorArray , &st ); er;
    tAttr.setStorable(false);
	st = addAttribute( aUp ); er;
	...
	eAttr.setKeyable(true);
	eAttr.setHidden(false);
	st = addAttribute( aInversions );er;
	...
	eAttr.setKeyable(true);
	eAttr.setHidden(false);
	st = addAttribute( aOutputType );er;
	
	
	aOut = tAttr.create("out", "o", MFnData::kVectorArray , &st ); er;
 	tAttr.setStorable(false);
	tAttr.setReadable(true);
	st = addAttribute( aOut ); er;
