...
// Initialisation and destruction
 ...
 axisSensor::initialize()
 {
 	...
 	MStatus st;
 	aAxisPP = tAttr.create("axisPP", "axpp", MFnData::kVectorArray, &st ); er;
 	tAttr.setStorable(false);
 	tAttr.setConnectable(true);
 	st = addAttribute(aAxisPP); er;

 	aPointNormal	= nAttr.create("pointNormal","pno", MFnNumericData::kDouble, 1.0, &st); er;
 	nAttr.setHidden(false);
 	nAttr.setKeyable(true);
 	nAttr.setStorable(true);
 	nAttr.setWritable(true);
 	st = addAttribute(aPointNormal); er;
	//--------------

 	aAxisNormal	= nAttr.create("axisNormal","ano", MFnNumericData::kDouble, 1.0, &st); er;
 	nAttr.setHidden(false);
 	nAttr.setKeyable(true);
 	nAttr.setStorable(true);
 	nAttr.setWritable(true);
 	st = addAttribute(aAxisNormal); er;
	//--------------

 	aAxisDirection	= nAttr.create("axisDirection","adr", MFnNumericData::kDouble, 1.0, &st); er;
 	nAttr.setHidden(false);
 	nAttr.setKeyable(true);
 	nAttr.setStorable(true);
 	nAttr.setWritable(true);
 	st = addAttribute(aAxisDirection); er;
	//--------------

 	aAxisOrbit	= nAttr.create("axisOrbit","aor", MFnNumericData::kDouble, 1.0, &st); er;
 	nAttr.setHidden(false);
 	nAttr.setKeyable(true);
 	nAttr.setStorable(true);
 	nAttr.setWritable(true);
 	st = addAttribute(aAxisOrbit); er;
	//--------------
			...
			////////////////////////////////
 			...
 			for (AGENT_VECTOR::const_iterator currAgent = neighbors->begin(); currAgent!=neighbors->end(); currAgent++ ) {
				...
				///////////////////
 				...
 				signal += ( (vPointNorm*pointNormVal) + (vAxisOrbit*axisOrbitVal) + (vAxisDir*axisDirVal)+ (vAxisNorm*axisNormVal) ) *atten ;
				///////////////////
