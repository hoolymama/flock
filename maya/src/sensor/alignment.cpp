...
alignment::initialize()
{
	...
	aMeasureVelocity = eAttr.create( "measureVelocity", "mvl", kAbsolute, &st ); er;
	eAttr.addField( "absolute",		kAbsolute);
	eAttr.addField( "relative",		kRelative	);
	eAttr.addField( "blend",		kBlend	);
	eAttr.setKeyable(true);
	eAttr.setStorable(true);
	st = addAttribute(aMeasureVelocity); er;

	
	
	aBlend	= nAttr.create("relativeBlend","rlb", MFnNumericData::kDouble, 0.0, &st); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	nAttr.setStorable(true);
	nAttr.setWritable(true);
	st = addAttribute(aBlend); er;
