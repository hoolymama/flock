...
slotSensor::initialize()
{
	...
	aSlotRadius	= nAttr.create("slotRadius","srd", MFnNumericData::kDouble, 1, &st); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	nAttr.setStorable(true);
	nAttr.setWritable(true);
	st = addAttribute(aSlotRadius); er;

	aSlotImportancePP = tAttr.create("slotImportancePP", "sipp", MFnData::kDoubleArray, &st ); er;
	tAttr.setStorable(false);
	tAttr.setConnectable(true);
	st = addAttribute(aSlotImportancePP); er;

	// aSlotPositionPP = tAttr.create("slotPositionPP", "sppp", MFnData::kVectorArray, &st ); er;
	// tAttr.setStorable(false);
	// tAttr.setConnectable(true);
	// st = addAttribute(aSlotPositionPP); er;

	aImportanceMargin	= nAttr.create("importanceMargin","imm", MFnNumericData::kDouble, 0.1, &st); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	nAttr.setStorable(true);
	nAttr.setWritable(true);
	st = addAttribute(aImportanceMargin); er;
	
	aMaxFreeSlots	= nAttr.create("maxFreeSlots","mfs", MFnNumericData::kInt, 10, &st); er;
	nAttr.setHidden(false);
	nAttr.setKeyable(true);
	nAttr.setStorable(true);
	nAttr.setWritable(true);
	st = addAttribute(aMaxFreeSlots); er;
...
MStatus slotSensor::assess(MDataBlock& data, const poolData * pd, MVectorArray &assessment)
{
	...
	const MDoubleArray & slotImportancePP = MFnDoubleArrayData(data.inputValue(aSlotImportancePP).data()).array();
	...
	bool doIt =  (num && (0.0 != acuity) && (0.0 != radius) && (0.0 != slotRadius) && (0.0 < fov ));
