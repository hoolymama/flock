...
inline MStatus getRequiredSensorData(
  ...
  MVectorArray &viewVectors)  {
	unsigned plen = (MFnVectorArrayData(data.inputValue(
	                                      sensor::aPositionPP).data()).length());
	unsigned vlen = (MFnVectorArrayData(data.inputValue(
	                                      sensor::aVelocityPP).data()).length());
	...
	unsigned viewlen = MFnVectorArrayData(data.inputValue(
	                                        sensor::aViewVectorPP).data()).length();
...
inline double attenuateSense(
  ...
  const double
  &fov,			// My field of view (0 is thin cone, 0.5 is hemisphere, 1.0 is sphere)
...
)
{
	...
	double result = 1.0;
	const double fovN = (fov * -2.0) +
	                    1.; // field of view mapped into dotprod space i.e. (0 -> 1) -to- (1 -> -1) i.e. (1=narrow cone -> -1=sphere)
...
)
{
	...
	float lookupFov;
	fovRamp.getValueAtPosition( float(angle), lookupFov, &st ); mser;

	double closeness = (dist / maxDist);
	if (closeness > 1.0) { closeness =  1.0; }
	float lookupDec;
	decRamp.getValueAtPosition( float(closeness), lookupDec, &st ); mser;
