...
MStatus rigParticle::setPlugValues(
  ...
  MTransformationMatrix::RotationOrder order
	...
	tx.setValue(translateValue.x); mser;
	ty.setValue(translateValue.y); mser;
	tz.setValue(translateValue.z); mser;
	...
	for (unsigned i = 0; i < numPlugs; i++) {
		...
		if (plugState == MPlug::kFreeToChange) {
			st = (userPlugs[i]).setValue(m_values[i]); mser;
		}
	}
	for (unsigned i = 0; i < numVectorPlugs; i++) {
		...
		if (plugState == MPlug::kFreeToChange) {
			...
			st = xPlug.setValue(m_vector_values[i].x); mser;
			st = yPlug.setValue(m_vector_values[i].y); mser;
			st = zPlug.setValue(m_vector_values[i].z); mser;
		}
	}
