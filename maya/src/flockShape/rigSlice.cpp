...
{	
	...
	const MIntArray sortedIds = MFnIntArrayData(data.inputValue(flockShape::aSortedIdPP).data()).array();
	unsigned len = sortedIds.length(); 
	const MIntArray idIndex = MFnIntArrayData(data.inputValue(flockShape::aIdIndexPP).data()).array();
  ...
  //  }
	...
	const MDoubleArray  activepp  = MFnDoubleArrayData(data.inputValue(flockShape::aActivePP).data()).array();
	...
	mayaMath::axis aimFrontAxis = mayaMath::axis(data.inputValue(flockShape::aRotateAimFrontAxis).asShort());
	mayaMath::axis  aimUpAxis = mayaMath::axis(data.inputValue(flockShape::aRotateAimUpAxis).asShort());
	/////////////////////////////////////////////////////////////////
	...
	MArrayDataHandle hInUserMap = data.inputArrayValue( flockShape::aUserMap, &st ); er;
	unsigned num = hInUserMap.elementCount();
	MIntArray isDoubleArray;
	for(unsigned i = 0;i < num; i++, hInUserMap.next()) {
		MDataHandle hCompound = hInUserMap.inputValue(&st ); er;	
		...
		compoundPlug = userMapPlug.elementByPhysicalIndex(i,&st);er;
	...
	MArrayDataHandle hInUserVectorMap = data.inputArrayValue( flockShape::aUserVectorMap, &st ); er;
	num = hInUserVectorMap.elementCount();
	MIntArray isVectorArray;
	for(unsigned i = 0;i < num; i++, hInUserVectorMap.next()) {
		MDataHandle hCompound = hInUserVectorMap.inputValue(&st ); er;	
		MDataHandle hUserVectorPP = hCompound.child( flockShape::aUserVectorPP );

		// if connected, get the data, otherwise make a 1 element array from the value of the target - plug
		compoundPlug = userVectorMapPlug.elementByPhysicalIndex(i,&st);er;
...
//	cerr << iaActivepp << endl;
	...
	if (ids_valid){
		for (unsigned i = 0;i<len;i++) {
			...
			for ( da_iter = userArrays.begin();da_iter != userArrays.end();da_iter++, arrayCount++) {
			...
			arrayCount = 0;
			for ( va_iter = userVectorArrays.begin();va_iter != userVectorArrays.end();va_iter++, arrayCount++) {
				//cerr "da iter refers to pointer " << *da_iter << endl;
				//const MDoubleArray & array = *da_iter;
			...
			if (rotType == 0) { // aim
				if (do_r) {r = rpp[index];} else {r = MVector::xAxis;}
				if (do_u) {u = rupp[index];} else {u = MVector::yAxis;}	
				particle = new rigParticle(particleId,t,r,u,s,fa,fva,aimFrontAxis,aimUpAxis);		
			} else	{
				if (do_r) {r = rpp[index];} else {r = MVector::zero;}
...
MStatus rigSlice::fortify(const rigSlice & target){
	...
	while (    (iter != m_stack.end())   &&  (target_iter != target.stack().end())   ){
		st = (*iter)->setFuture(*target_iter); ert;
		iter++;
		target_iter++;
	}
...
void rigSlice::info() const {
	...
	while (iter != m_stack.end()) {
		cerr << "id:["<< (*iter)->id() << "] -- Matrix: " << (*iter)->transform() << " -- Values: " <<  (*iter)->values() << endl;
		iter++;
	}
}
