...
MStatus Ground::setMesh(const  MObject& node,  MDataBlock& data) {
  ...
  MObject aSnapFeetToMesh = dn.attribute( "snapFeetToMesh", &st);er;
 	m_useMesh = data.inputValue(aSnapFeetToMesh).asBool();

  MObject aAnimatedFloor = dn.attribute	( "animatedFloor", &st);er;
 	m_isAnimated = data.inputValue(aAnimatedFloor).asBool();



	MObject aFloorMesh = dn.attribute	( "floorMesh", &st);er;
